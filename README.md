...
Скрипты запуска лежат в папке `scripts`.
...
из папки `scripts`, и из корня, если положить его туда.
...
Он выдаёт адрес вида `https://…​.trycloudflare.com`. Дальше поднимаем бота
опросом — вебхук туннелю не нужен, а адрес всё равно меняется при каждом
перезапуске:

```powershell
.venv\Scripts\python.exe -m app.poll_bot --platform max
```

Опрос сам снимает все текущие подписки бота на вебхук (MAX не отдаёт события
одновременно двумя способами) и дальше слушает `GET /updates`.

URL мини-приложения MAX не даёт прописать через API — впишите адрес туннеля
руками один раз в кабинете «MAX для партнёров»: Чат-боты → ваш бот → ⋮ →
Настройки → поле URL мини-приложения → Сохранить. Требования к адресу: только
`https`, латиница/цифры/точка/дефис, до 1024 символов. После следующего
перезапуска туннеля адрес там придётся поменять — для локальной отладки это
разовое неудобство, на боевом стенде домен уже не меняется.

Каждому разработчику нужен **свой** бот: платформа отдаёт события только
одному получателю, и два человека с одним токеном будут перехватывать
сообщения друг у друга.

Опрос умеет обе платформы — `--platform telegram` поднимет отладочный
Telegram-адаптер, если он вам нужен (см. таблицу платформ в разделе
«Чат-бот»). Если задан только один из двух токенов, платформа определяется
сама и флаг можно не указывать.
...
сказано иное. Короткая версия без пояснений — в [docs/DEPLOY.md](docs/DEPLOY.md), там же
сценарий обновления уже поднятого стенда.
...
### 5. Подписка на вебхук и мини-приложение

Когда стенд поднят и домен отвечает по HTTPS, подписываем бота на вебхук:

```bash
docker compose exec backend python -m app.setup_bot --webhook https://example.ru
```

URL мини-приложения этой командой не задать — в отличие от Telegram, MAX не
даёт сделать это через API. Впишите адрес руками один раз в кабинете
«MAX для партнёров»: Чат-боты → ваш бот → ⋮ → Настройки → поле URL
мини-приложения → выберите вид кнопки → Сохранить. Требования к адресу: только
`https`, латиница/цифры/точка/дефис, до 1024 символов.

Платформа MAX (боты и мини-приложения) доступна юрлицам, ИП и самозанятым —
резидентам РФ; без пройденной верификации в кабинете партнёра мини-приложение
не заработает у посторонних пользователей.
...
| `MAX_CA_BUNDLE` / `MAX_VERIFY_SSL` | MAX тоже подписан НУЦ Минцифры — сертификат уже вшит в репозиторий (`app/certs/`) и подключается сам, заполнять не нужно. Переменные — только для переопределения пути или (для отладки) отключения проверки |
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_BOT_USERNAME` / `TELEGRAM_WEBHOOK_SECRET` | Необязательный отладочный адаптер. Пустой токен → выключен полностью |
| `APP_PUBLIC_URL` | Адрес мини-приложения — на него подписывается вебхук. Для MAX кнопку в самом мессенджере нужно прописать вручную (см. «Деплой на сервер», шаг 5) |
| `CORS_ALLOWED_ORIGINS` | Origin, которым разрешён доступ к API. По умолчанию `*` (всем) — сузить до домена мини-приложения, когда он известен: один адрес или несколько через запятую |
...
|---|---|---|
| MAX | `POST /api/bot/max` | целевая; вход в мини-приложение проверяет подпись initData |
| Telegram | `POST /api/bot/telegram` | необязательный адаптер для отладки сценариев, включается только при заданном `TELEGRAM_BOT_TOKEN` |
...
Подписка на вебхук — одной командой для обеих платформ сразу
(`app/setup_bot.py`, см. «Деплой на сервер», шаг 5):

```bash
python -m app.setup_bot --webhook https://<домен>
```
...
# вебхук MAX
curl -X POST http://localhost/api/bot/max \
